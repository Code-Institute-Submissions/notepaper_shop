...
def get_cart_items_and_total(cart, discount):
    cart_items = []
    total = 0
    ...
    if discount_percent > 0:
        discount_percent = discount_percent/Decimal(100)
        total = Decimal(total)
        total = total - (total * discount_percent).quantize(Decimal('.01'))
        # total=Decimal(total-(total*(discount_percent))).quantize(Decimal('.01'))
